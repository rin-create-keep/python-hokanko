...
def add_tasks_from_input(titles_str, cat, prio, dl):
    """
    titles_str: セミコロン区切りで複数タイトルを受け取る（例: タスク1;タスク2）
    cat: カテゴリ（空なら '未分類'）
    prio: 優先度（1-4）
    dl: 期限（YYYY-MM-DD または None）
    """
    if not titles_str:
        return 0
...
def import_from_uploaded_file(uploaded_file):
    """
    期待されるファイル形式（UTF-8 テキスト）:
      各行: タイトル,カテゴリ,優先度,期限
    期限は省略可。優先度は 1-4 の整数（省略したら 3）.
    行頭に # がある場合はコメント扱いで無視。
    """
...
def ui_add_and_import_and_search():
    ...
    if st.sidebar.button("検索実行"):
        ...
        if not filtered:
            ...
            # 検索結果を一時表示エリアに保存してメインで参照できるようにする
